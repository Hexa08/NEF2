...
def _load_cuda_lib():
    ...
    if system == "Windows":
        ...
        raise CudaError("macOS does not support CUDA. Use Metal backend.")
    else:
...
class CudaRuntime:
    def __init__(self, index=0):
        self._rt = runtime(index)
        self.count = self._rt.count
    def name(self): return self._rt.name()

class CudaTensor:
    def __init__(self, data, device=None):
        self.device = _CURRENT_DEVICE if device is None else device
        self._rt = runtime(self.device)
        from ...tensor import _flatten, _shape
        flat = _flatten(data)
    ...
    def matmul(self, other):
        ...
        from ...tensor import _prod
        batch_count = _prod(self.shape[:-2]) if len(self.shape) > 2 else 1
        b_batched = 1 if len(other.shape) > 2 else 0
        total = batch_count * m * n
        self._rt.launch(
            "matmul",
            [
                ("u64", self.ptr), ("u64", other.ptr), ("u64", out.ptr),
                ("u32", batch_count), ("u32", m), ("u32", k), ("u32", n), ("u32", b_batched),
            ],
            total,
        )
        return out
    ...
    def layernorm(self, gamma, beta, eps=1e-5):
        if not isinstance(gamma, CudaTensor): gamma = CudaTensor(gamma, self.device)
        if not isinstance(beta, CudaTensor): beta = CudaTensor(beta, self.device)
        ...
        self._rt.launch(
            "layernorm_fwd",
            [
                ("u64", self.ptr), ("u64", gamma.ptr), ("u64", beta.ptr), ("u64", out.ptr),
                ("u32", n_rows), ("u32", n_features), ("f32", eps),
            ],
            n_rows,
        )
        return out
